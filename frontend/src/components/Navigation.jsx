...
export function Navigation({ activePage, onNavigate }) {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const cleanedIdCandidate = rawTerm
        .replace(/^id[:\s-]*/i, "")
        .replace(/\s+/g, "")
        .replace(/[^A-Za-z0-9-]/g, "");
      const isCustomId = /^[A-Za-z]{1,6}-?\d{2,8}$/i.test(cleanedIdCandidate);
      const searchTerm = isCustomId
        ? cleanedIdCandidate.toUpperCase()
        : rawTerm;
  ...
  const { logout, user } = useContext(AuthContextr);
      ...
      <div className="w-full mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-2 md:py-4">
        <div className="flex items-center justify-between w-full">
          ...
          <div className="flex items-center flex-shrink-0 gap-2 lg:gap-4 pr-3 lg:pr-6 min-w-[56px] max-w-full overflow-visible">
            ...
            <div
              className="p-0 m-0 flex items-center justify-center min-w-[44px] min-h-[44px] overflow-visible"
              style={{
                marginRight: "8px",
              }}
            >
            ...
            <div className="hidden sm:block">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  ...
                  >
                    <Avatar className="size-10 lg:size-9 xl:size-8 ring-2 ring-white">
                      <AvatarImage
                        src={
                          profileUser?.closerPhotoUrl ||
                          profileUser?.closerPhoto?.url ||
                ...
                <DropdownMenuContent
                  ...
                  side="bottom"
                  sideOffset={8}
                  className="w-56 p-1 border border-[#e5e7eb] rounded-[12px] shadow-lg z-[110]"
                >
                  <div className="px-3 py-2 border-b border-[#e5e7eb]">
                    <p className="text-sm font-semibold text-[#111] capitalize line-clamp-1">
                  ...
                  <div className="py-1">
                    ...
                    <DropdownMenuItem
                      ...
                      className="rounded-lg px-3 py-2 text-sm font-medium text-[#b91c1c] hover:bg-red-50 cursor-pointer"
                    >
                      Logout
                    </DropdownMenuItem>
                  </div>
            ...
            <div className="block lg:hidden z-50">
              <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
                ...
                >
                  ...
                  <div
                    className="flex flex-col gap-3 pt-4 pb-6 px-4 overflow-y-auto bg-white"
                    style={{
                  ...
                  >
                    ...
                    <button
                      onClick={() => handleNavigation("logout")}
                      className="w-full text-left px-4 py-3 rounded-lg bg-red-50 hover:bg-red-100 transition-all font-medium text-[#b91c1c] flex items-center gap-2"
                    >
                      <span>🚪</span> Logout
                    </button>
                  </div>
