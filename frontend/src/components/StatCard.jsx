...
export function StatCard({
  ...
  return <div className="bg-white rounded-[20px] p-4 md:p-5 lg:p-6 satfera-shadow hover:shadow-lg transition-shadow">
      <div className="flex flex-col gap-3">
        <div>
          <p className="text-sm text-muted-foreground mb-1 m-0">{label}</p>
          <div className="flex items-baseline gap-2">
            <span className="text-[#222222] m-0 text-2xl md:text-3xl lg:text-[2rem]" style={{
            fontWeight: 600,
            fontFamily: 'Playfair Display, serif'
          }}>
              {value}
            </span>
            {metric && <span className="text-xs md:text-sm text-muted-foreground">{metric}</span>}
          </div>
        </div>
