  ...
  return <div className="w-full flex flex-col items-center px-3 sm:px-6">
      ...
      <div className="w-full overflow-x-auto scrollbar-hide nav-progres-bar flex justify-center max-sm:block">
        <div className="relative flex items-center gap-4 sm:gap-6 md:gap-8 px-4 min-w-fit">
          ...
          {}
          <div className="absolute top-[35%] left-0 h-[2px] bg-[var(--brand-step-done)] z-[1] transition-all duration-500" style={{
          ...
          return <div key={step.id} onClick={() => onStepClick(step.id)} className="flex flex-col items-center text-center relative z-10 cursor-pointer select-none">
                {}
                <div className={`flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-full border-2 transition-all duration-300 ${isDone ? "bg-[var(--brand-step-done)] border-[var(--brand-step-done)] text-white" : isActive ? "border-[var(--brand-primary)] text-[var(--brand-primary)] bg-white" : "border-gray-300 text-gray-500 bg-white"}`}>
                  {isDone ? <Check size={18} /> : index + 1}
                </div>

                {}
                <span className={`mt-2 text-[10px] sm:text-[12px] font-medium leading-tight whitespace-nowrap ${isDone ? "text-black" : isActive ? "text-[var(--brand-primary)]" : "text-gray-700"}`}>
                  {step.label}
                </span>
              </div>;
        })}
