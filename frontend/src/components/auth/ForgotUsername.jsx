  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (response.success && response.data?.username) {
        ...
        setError(response.message || "If an account exists with the provided details, the username will be displayed.");
      }
    } catch (err) {
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8">
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            ...
            {}
            {error && <div role="alert" aria-live="assertive" className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg text-sm">
                {error}
              </div>}
          ...
          {}
          {username && <div role="alert" aria-live="polite" className="mt-6 bg-green-50 border border-green-200 rounded-lg p-4">
