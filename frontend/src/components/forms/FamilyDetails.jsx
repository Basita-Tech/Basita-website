...
const FamilyDetails = ({
  ...
  const handleChange = useCallback(e => {
    ...
    const formattedValue = name.toLowerCase().includes("name") || name.toLowerCase().includes("native") || name.toLowerCase().includes("profession") ? formatFullName(value) : value;
    setFormData(prev => ({
      ...prev,
      [name]: formattedValue
    }));
  ...
  const handleSubmit = async e => {
    ...
    if (formData.hasSiblings === true) {
      ...
      submissionData.siblingDetails = (formData.siblings || []).filter(s => s.name?.trim() && s.relation?.trim());
      ...
      <div className="bg-[#FBFAF7] shadow-2xl rounded-3xl w-full max-w-xl p-4 sm:p-8 border-t-4 border-[#F9F7F5] transition-transform duration-300">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            {["Name", "Profession", "Native"].map(field => <div className="flex flex-col" key={field}>
          ...
          <div className="space-y-4">
            ...
            {["Name", "Profession"].map(field => <div className="flex flex-col" key={field}>
                <label className="text-sm font-medium mb-1">
                  Mother's {field}
                </label>
          ...
          <div className="space-y-4 mt-4">
            ...
            <div className="flex gap-4 items-center">
              {["Yes", "No"].map(option => <label key={option} className="flex items-center gap-2 cursor-pointer text-sm">
                  <input type="radio" name="hasSiblings" value={option} checked={formData.hasSiblings === (option === "Yes")} onChange={() => setFormData(prev => ({
                ...prev,
                hasSiblings: option === "Yes",
                siblingCount: 0,
                siblings: []
              }))} onKeyDown={e => handleRadioKeyDown(e, () => setFormData(prev => ({
                ...prev,
                hasSiblings: option === "Yes",
                siblingCount: 0,
                siblings: []
            ...
            {formData.hasSiblings && <div className="mt-3 space-y-3">
