...
const HealthLifestyle = ({
  ...
  const validateForm = useCallback(() => {
    const newErrors = {};
    const requiredFields = ["alcohol", "tobacco", "tattoos", "hiv", "tb", "medicalHistory", "diet"];
    requiredFields.forEach(field => {
      if (!formData[field]) {
        newErrors[field] = "This field is required";
      }
    });
    if (formData.medicalHistory === "yes" && !formData.medicalHistoryDetails.trim()) {
      ...
      <div className="bg-[var(--brand-card)] shadow-2xl rounded-3xl w-full max-w-xl p-4 sm:p-8 border-t-[2px] border-[var(--brand-bg)] transition-transform duration-300">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex flex-col">
            ...
            {formData.medicalHistory === "yes" && <textarea value={formData.medicalHistoryDetails} onChange={e => handleChange("medicalHistoryDetails", e.target.value)} placeholder="Please specify in under 100 words" maxLength={500} className={`${inputClass} mt-2 capitalize`} />}
          </div>
