...
export function Dashboard({
    ...
    async function fetchDashboardData() {
      ...
      try {
        ...
        const completedSteps =
          onboardingData?.completedSteps || userData?.completedSteps || [];
        ...
        const profileStatus =
          onboardingData?.profileReviewStatus || userData?.profileReviewStatus;
        ...
        if (error?.response?.status === 403) {
          toast.error(
            error.response?.data?.message || "Profile approval required",
          );
          navigate("/onboarding/review", { replace: true });
          return;
        }
  ...
  useEffect(() => {
    ...
    const fetchRecommendations = async () => {
      ...
      try {
        ...
        const mapped = data.map((match) => {
          ...
          return {
            ...
            profession:
              userData.profession ||
              userData.occupation ||
              userData.professional?.Occupation,
        ...
        setTotalMatches(
          response?.pagination?.total ?? response?.total ?? mapped.length,
        );
      } catch (err) {
  ...
  const filteredRecommendations = useMemo(() => {
    ...
    return recommendations.filter(
      (profile) => !sentIds.has(String(profile.id)),
    );
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-4 md:px-8 py-6 space-y-8">
      <div className="bg-white rounded-[20px] p-4 md:p-6 lg:p-8 shadow-sm">
        <div className="flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-6">
          <div className="relative w-24 h-24 md:w-28 md:h-28 shrink-0">
            {!imageLoaded && user?.closerPhotoUrl && (
              <div className="absolute inset-0 bg-gradient-to-br from-gray-200 to-gray-300 rounded-xl animate-pulse" />
            )}
            <img
              src={
                user?.closerPhotoUrl ||
              ...
              alt="Your Profile"
              id="dashboard-photo"
              className={`w-full h-full rounded-xl border border-gray-200 shadow-sm object-cover transition-opacity ${imageLoaded ? "opacity-100" : "opacity-0"}`}
          ...
          <div className="flex flex-col text-center md:text-left">
            <div className="flex flex-col md:flex-col items-center md:items-start">
              <div className="flex items-center gap-3 text-center flex-wrap justify-center md:justify-start">
      ...
      <div ref={recommendationsSectionRef} className="space-y-4">
        ...
        {!recommendationsLoading && totalMatches > 0 && (
