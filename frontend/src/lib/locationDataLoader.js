// This module is lazy-loaded only when location data is actually needed
// This prevents country-state-city from bloating the main bundle
...
export const getIndianStates = () => {
  try {
    return State.getStatesOfCountry("IN").map(s => s.name).sort();
  } catch (error) {
...
export const getIndianCities = () => {
  try {
    ...
    console.error('Error getting Indian cities:', error);
    return [];
  }
};
...
export const getCitiesData = (countryCode, stateCode) => {
  if (!countryCode || !stateCode) return [];
  try {
    const cities = City.getCitiesOfState(countryCode, stateCode);
    return (cities || []).map(c => ({
    ...
    console.error(`Error loading cities for ${countryCode}-${stateCode}:`, error);
    return [];
  }
};
